...
export default function AdminBusinessDetail() {
  ...
  return (
    <Layout title={business.name} subtitle={`${business.business_type} · ${business.owner_name}`}
      action={<Button variant="outline" className="rounded-xl" data-testid="back-to-businesses" onClick={() => navigate("/admin/umkm")}>
        <ArrowLeft className="h-4 w-4 mr-1.5" /> Kembali</Button>}>
      <Tabs value={tab} onValueChange={(v) => setParams({ tab: v })}>
        ...
        <TabsContent value="dashboard"><DashboardView businessId={businessId} adminMode /></TabsContent>
        <TabsContent value="transaksi"><TransactionsView businessId={businessId} isAdmin /></TabsContent>
